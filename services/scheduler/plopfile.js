...
const routesLocation = path.join(__dirname, './src');
const containerLocation = path.join(__dirname, './src/container.ts');

const directories = getDirectories(`${routesLocation}/app/features`)
...
const createRouting = {
  type: 'add',
  path: `${routesLocation}/app/features/{{name.kebabCased}}/routing.ts`,
...
const updateRootRouter = [{
  type: 'modify',
  path: `${routesLocation}/app/applications/http/router.ts`,
...
module.exports = plop => {
  plop.setActionType('makeDir', function (answers, {configProp}) {
    const absolutePath = path.join(`${routesLocation}/app/features`, answers.name.kebabCased, configProp);
